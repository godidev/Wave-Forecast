...
async function fetchSurfForecast(beach = 'Sopelana'): Promise<string> {
  ...
  const options = {
    ...
    headers: {
      ...
      'x-csrf-token': 'fX2oQTDp4Y0cUt8YM8H3V/eKNP1X/Iy2MTOyPLACKmiTLblSBmy78AVmz3xoOB8tbPTyybzKlr1nn6sYZs5teg=='
    }
  }
  try {
...
async function parseForecast(html: string) {
  ...
  waves.each((index, element) => {
    ...
    const { period, angle: waveDirection, height } = dataArray.find((item: unknown) => item !== null) || {}
    const { speed, direction } = JSON.parse($(element).attr('data-wind') as string)
...
function getDate(date: string): number[] {
  ...
  const match = /^(\d+)(AM|PM)$/.exec(hour)
  if (match) {
    const [, time, period] = match
    const parsedTime = parseInt(time)
    if (period === 'PM') {
      return parsedTime === 12 ? [parsedDay, parsedTime] : [parsedDay, parsedTime + 12]
    }
